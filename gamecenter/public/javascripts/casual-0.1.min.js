...
(function() {
	...
	c.prototype.concat = function(a) {
		var b = this.a, d = this.c, e = this.tx;
	...
	c.prototype.invert = function() {
		var a = this.a, b = this.b, d = this.c, e = this.d, f = this.tx, g = a
				* e - b * d;
		this.a = e / g;
		this.b = -b / g;
		this.c = -d / g;
...
(function() {
	...
	c.prototype.intersects = function(a) {
		return this.x <= a.x + a.width && a.x <= this.x + this.width
				&& this.y <= a.y + a.height && a.y <= this.y + this.height
	};
	c.prototype.intersection = function(a) {
		var b = Math.max(this.x, a.x), d = Math.min(this.x + this.width, a.x
				+ a.width);
		if (b <= d) {
			var e = Math.max(this.y, a.y);
			a = Math.min(this.y + this.height, a.y + a.height);
			if (e <= a)
				return new c(b, e, d - b, a - e)
		}
		return null
	};
	c.prototype.containsPoint = function(a, b) {
		return this.x <= a && a <= this.x + this.width && this.y <= b
...
(function() {
	...
	c.displayObjectToString = function(a) {
		...
		for (a = a; a != null; a = a.parent) {
			...
			b = b == null ? d : d + "." + b;
			if (a == a.parent || a.parent instanceof Stage)
				break
		}
		return b
	}
...
(function() {
	...
	c.prototype.toString = function() {
		return "[StageEvent type=" + this.type + ", mouseX=" + this.mouseX
				+ ", mouseY=" + this.mouseY + "]"
	}
})();
(function() {
	...
	c.addEventListener = function(a, b, d) {
		...
		var f = c._sources[a];
		if (f == undefined)
			f = c._sources[a] = c._counter++;
		if (e[f] == undefined)
			e[f] = {};
		e = e[f];
		a = c._listeners[d];
		if (a == undefined)
			a = c._listeners[d] = c._counter++;
		if (!(a in e)) {
			e = e[a] = {};
			e.type = b;
			e.src = f;
...
(function() {
	...
	c.prototype.getConcatenatedMatrix = function() {
		for ( var b = new casual.Matrix, d = this; d != null; d = d.parent) {
			b.concatTransform(d.x, d.y, d.scaleX, d.scaleY, d.rotation, d.regX,
					d.regY);
			if (d instanceof casual.Stage)
				break
		}
		return b
	};
	...
	c.prototype.hitTestPoint = function(b, d, e, f) {
		if (!e) {
			...
			return f.x >= 0 && f.x <= this.getCurrentWidth() && f.y >= 0
		...
		b = false;
		try {
			if (e.getImageData(0, 0, 1, 1).data[3] >= f)
				b = true
		} catch (g) {
	...
	c.prototype.hitTestObject = function(b, d, e) {
		...
		if (f && f.width > 0 && f.height > 0) {
			...
			try {
				...
				for (b = 0; b < i.length;) {
					if ((i[b] > 0 || i[b + 1] > 0 || i[b + 2] > 0 || i[b + 3] >= e)
							&& (j[b] > 0 || j[b + 1] > 0 || j[b + 2] > 0 || j[b + 3] >= e)) {
						d = true;
						break
					}
					b += 4
	...
	c.prototype.getRect = function(b) {
		...
		return new Rectangle(b.x, b.y, this.getCurrentWidth(), this
				.getCurrentHeight())
	};
	...
	c.prototype.getObjectUnderPoint = function(a, b, d, e, f) {
		...
		for ( var h = this.children.length - 1; h >= 0; h--) {
			...
			if (!(i == null || !i.mouseEnabled || !i.visible || i.alpha <= 0))
				if (i instanceof c && i.mouseChildren && i.getNumChildren() > 0) {
					...
					if (j)
						if (f) {
							...
							return j;
					else if (i.hitTestPoint(a, b, d, e))
						if (f)
							g.push(i);
						else
							return i
...
(function() {
	var c = function(a) {
		...
		if (casual.StageEvent.supportTouch()) {
			this.canvas.addEventListener("touchstart", casual.delegate(
					this.__touchHandler, this), false);
			this.canvas.addEventListener("touchmove", casual.delegate(
					this.__touchHandler, this), false);
			this.canvas.addEventListener("touchend", casual.delegate(
					this.__touchHandler, this), false)
		} else {
			this.canvas.addEventListener("mousedown", casual.delegate(
					this.__mouseHandler, this), false);
			this.canvas.addEventListener("mousemove", casual.delegate(
					this.__mouseHandler, this), false);
			this.canvas.addEventListener("mouseup", casual.delegate(
					this.__mouseHandler, this), false)
		}
	};
	...
	c.prototype.setFrameRate = function(a) {
		if (this._frameRate != a) {
			...
			this.__intervalID = setInterval(casual.delegate(this.__enterFrame,
					this), 1E3 / this._frameRate)
		}
	};
	c.prototype.__touchHandler = function(a) {
		var b = a.touches && a.touches.length ? a.touches[0] : a.changedTouches
				&& a.changedTouches.length ? a.changedTouches[0] : null;
		if (b) {
			var d = b.pageY || b.clientY || b.screenY;
			this.mouseX = (b.pageX || b.clientX || b.screenX)
		...
		if (this.mouseTarget && this.mouseTarget.onMouseEvent) {
			...
			b.type == "mouseup"
					&& this.mouseTarget.onMouseEvent(casual.copy(a,
							casual.StageEvent, {
								type : "mouseout"
							}))
		}
	...
	c.prototype.__mouseHandler = function(a) {
		...
		this.mouseTarget && this.mouseTarget.onMouseEvent
				&& this.mouseTarget.onMouseEvent(b);
		this
	...
	c.prototype.__getMouseTarget = function(a) {
		var b = this.getObjectUnderPoint(this.mouseX, this.mouseY,
				this.usePixelTrace), d = this.mouseTarget;
		this.mouseTarget = b;
	...
	c.prototype.__enterFrame = function() {
		if (!(this._paused && !this._pauseInNextFrame)) {
			...
			if (!this._paused || this._pauseInNextFrame)
				this._render(this.context, true);
	...
	c.prototype.clear = function(a, b, d, e) {
		arguments.length >= 4 ? this.context.clearRect(a, b, d, e)
...
(function() {
	...
	casual.Bitmap = c;
	c.prototype.render = function(a) {
		a.drawImage(this.image, this.frame[0], this.frame[1], this.frame[2],
				this.frame[3], 0, 0, this.width, this.height)
	}
})();
(function() {
	var c = function() {
...
(function() {
	...
	c.prototype.addFrame = function(a) {
		if (a[0] instanceof casual.Frame || a[0] instanceof Array)
			for ( var b in a)
				this.addFrame(a[b]);
		else
			this.setFrame(this._frames.length + 1, a)
	};
	...
	c.prototype.getFrameNumber = function(a) {
		...
		var b = a;
		if (typeof b == "string")
			b = this._frameLables[a];
		for (a = 0; a < this._frames.length; a++)
			if (b == this._frames[a])
				return a + 1;
		return -1
	};
	...
	c.prototype.nextFrame = function() {
		...
		if (a.pauseFrames)
			if (a.pauseFrames > this._pauseFrames)
				this._pauseFrames++;
			else
				this._pauseFrames = 0;
		if (a.gotoFrame)
			if (this._pauseFrames == 0 || !a.pauseFrames)
				return this.currentFrame = this.getFrameNumber(a.gotoFrame);
		return a.pauseFrames && this._pauseFrames > 0 ? this.currentFrame
				: this.currentFrame >= this._frames.length ? this.currentFrame = 1
						: ++this.currentFrame
	};
	...
	c.prototype.render = function(a) {
		...
		this._frameDisObj && this._frameDisObj != b.disObj
...
(function() {
	...
	c.prototype.lineStyle = function(a, b, d, e, f, g) {
		...
		if (f != undefined)
			this._context.lineJoin = f;
		if (g != undefined)
			this._context.miterLimit = g;
		return this
	};
	...
	c.prototype.beginFill = function(a, b) {
		if (a)
			this.fillStyle = this._context.fillStyle = a;
		this.fillAlpha = b || 1;
		return this
	};
	...
	c.prototype.drawEllipse = function(a, b, d, e) {
		if (d == e)
			return this.drawCircle(a, b, d);
		d = d / 2;
		e = e / 2;
		var f = 0.5522847498307933 * d, g = 0.5522847498307933 * e;
		a += d;
		b += e;
		this.beginPath();
		this.moveTo(a + d, b);
	...
	c.prototype.setSize = function(a, b) {
		this._context.save();
		if (a != undefined)
			this._context.canvas.width = a;
		if (b != undefined)
			this._context.canvas.height = b;
		this._context.restore();
		return this
	};
	c.prototype.clear = function() {
		this._context.clearRect(0, 0, this._context.canvas.width,
...
(function() {
	...
	c.prototype.render = function(a) {
		if (!(!this.text || this.text.length == 0)) {
			...
			this.outline ? a.strokeText(this.text, 0, 0, this.maxWidth) : a
					.fillText(this.text, 0, 0, this.maxWidth)
		}
	};
...
(function() {
	...
	c.prototype.setEnabled = function(a) {
		if (this.enabled == a)
			return this;
		if (this.mouseEnabled = this.enabled = a)
			this.currentFrame == 4 && this.gotoAndStop(c.state.UP);
		else
			this.disabledState ? this.gotoAndStop(c.state.DISABLED) : this
					.gotoAndStop(c.state.UP);
		return this
	};
	c.prototype.setState = function(a) {
		if (this.state != a) {
			...
			case c.state.UP:
				if (!this.enabled)
					this.mouseEnabled = this.enabled = true;
				this.gotoAndStop(a);
				break;
		...
		if (this.enabled)
			...
			case "mousemove":
				if (this.onMouseOver && this.state != c.state.OVER) {
					...
					this.onMouseMove && this.state == c.state.OVER
							&& this.onMouseMove(a);
				this.overState && this.state == c.state.UP
						&& this.setState(c.state.OVER);
				break;
			case "mouseout":
			...
			case "mousedown":
				...
				break;
			case "mouseup":
				this.overState ? this.setState(c.state.OVER) : this
						.setState(c.state.UP);
...
(function() {
	var c = function(a, b, d) {
		...
		try {
			...
			this._element.src = a;
			this._element.load();
			this._element.addEventListener("ended", casual.delegate(
					this._endHandler, this), false);
			this._loadInterval = setInterval(casual.delegate(this._loadHandler,
					this), 10)
		} catch (e) {
		}
	};
